...
// 確信度閾値: 50 → 65 に引き上げ (2026-05-30, profit 化試行)。
// データ的に conf 80-89% bucket が損失 93% を占めてたので、calibration 後の
// 確信度がさらに高い setup のみ通す = 取引数は減るが質を上げる狙い。
// 構造的に edge がない以上、改善幅は marginal の可能性が高い。
const LIVE_CONFIDENCE_THRESHOLD = 65;
